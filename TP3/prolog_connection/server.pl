...
server :-
	...
	socket_server_open(Port, Socket),  % 1.
	server_loop(Socket),
...
% GET /test(2,5) HTTP/1.1
% RL é "test(2,5) ", RL2 é "test(2,5)."
% ASCII 32 = space " "     ASCII 46 = dot "."
...
parse_input(available_moves(GameState, Player, X-Y), Res) :- available_moves(GameState, Player, X-Y, Res).
parse_input(spot(GameState, Player, X-Y), Res):- spot(GameState, Player, X-Y, Res).
parse_input(moveto(GameState, Player, X-Y-Xf-Yf), Res):- moveto(GameState, Player, X-Y-Xf-Yf, Res).
parse_input(move(GameState, X-Y-Xf-Yf), Res):- move(GameState, X-Y-Xf-Yf, Res).
...
% cria uma lista com N elementos, todos C
test(_,[],N) :- N =< 0.
